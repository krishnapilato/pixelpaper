  ...
  try {
    ...
    return cameras!.map((Map<dynamic, dynamic> camera) {
      return CameraDescription(
        ...
        manualSensor: camera['manualSensor'] == true,
        isoRange: camera['isoMin'] is int && camera['isoMax'] is int
            ? (min: camera['isoMin'] as int, max: camera['isoMax'] as int)
            : null,
        exposureRangeNs:
            camera['exposureMinNs'] is int && camera['exposureMaxNs'] is int
                ? (
                    min: camera['exposureMinNs'] as int,
                    max: camera['exposureMaxNs'] as int,
                  )
                : null,
        minFocusDiopters:
            (camera['minFocusDiopters'] as num?)?.toDouble() ?? 0,
        hasFlash: camera['flash'] == true,
        exposureCompensationRange:
            camera['aeCompensationMin'] is int && camera['aeCompensationMax'] is int
                ? (
                    min: camera['aeCompensationMin'] as int,
                    max: camera['aeCompensationMax'] as int,
                  )
                : null,
        exposureCompensationStep:
            (camera['aeCompensationStep'] as num?)?.toDouble() ?? 0,
        maxZoom: (camera['maxZoom'] as num?)?.toDouble() ?? 1,
        whiteBalancePresets: [
          for (final mode in (camera['awbModes'] as List?) ?? const [])
            if (_whiteBalanceFromAwbMode(mode as int) case final preset?)
              preset,
        ],
      );
    }).toList();
...
/// Maps Camera2's `CONTROL_AWB_MODE_*` constants to the presets (PixelPaper
/// patch).
WhiteBalancePreset? _whiteBalanceFromAwbMode(int mode) => switch (mode) {
      0 => WhiteBalancePreset.off,
      1 => WhiteBalancePreset.auto,
      2 => WhiteBalancePreset.incandescent,
      3 => WhiteBalancePreset.fluorescent,
      4 => WhiteBalancePreset.warmFluorescent,
      5 => WhiteBalancePreset.daylight,
      6 => WhiteBalancePreset.cloudy,
      7 => WhiteBalancePreset.twilight,
      8 => WhiteBalancePreset.shade,
      _ => null,
    };
...
class CameraDescription {
  CameraDescription({
    ...
    this.isoRange,
    this.exposureRangeNs,
    this.minFocusDiopters = 0,
    this.hasFlash = false,
    this.whiteBalancePresets = const [],
    this.exposureCompensationRange,
    this.exposureCompensationStep = 0,
    this.maxZoom = 1,
  });
  ...
  /// PixelPaper patch: true when the lens honours manual ISO, shutter and
  /// focus (Camera2's MANUAL_SENSOR capability).
  final bool manualSensor;

  /// PixelPaper patch: the sensor's ISO range, when reported.
  final ({int min, int max})? isoRange;

  /// PixelPaper patch: the sensor's exposure time range in nanoseconds.
  final ({int min, int max})? exposureRangeNs;

  /// PixelPaper patch: the closest focus distance in dioptres (1/metres); 0
  /// means the lens is fixed-focus.
  final double minFocusDiopters;

  /// PixelPaper patch: whether the lens has a flash unit (used as a torch).
  final bool hasFlash;

  /// PixelPaper patch: the white balance presets the lens supports.
  final List<WhiteBalancePreset> whiteBalancePresets;

  /// PixelPaper patch: exposure compensation range, in steps of
  /// [exposureCompensationStep] EV (e.g. -6..6 at 1/3 EV is -2..+2 EV).
  final ({int min, int max})? exposureCompensationRange;

  /// PixelPaper patch: EV per compensation step; 0 when not supported.
  final double exposureCompensationStep;

  /// PixelPaper patch: the largest digital zoom ratio; 1 means none.
...
class CameraController extends ValueNotifier<CameraValue> {
  ...
  // PixelPaper patch: nullable instead of `late`, so dispose() works on a
  // controller whose initialize() failed or never ran — 0.1.0 threw a
  // LateInitializationError there, or waited forever on a completer that a
  // failed initialize() never completed.
  ...
  Future<void> initialize() async {
    ...
    final creating = _creatingCompleter = Completer<void>();
    try {
      final Map<String, dynamic>? reply =
        ...
        <String, dynamic>{
          ...
          'iso': iso,
          'shutterSpeed': _frameDurationNs(shutterSpeed),
          'whiteBalance': serializeWhiteBalancePreset(whiteBalance),
          'focusDistance': _diopters(focusDistance),
        },
      );
      _textureId = reply!['textureId'];
      if (_isDisposed) {
        // Disposed while opening: dispose() is waiting on this completer and
        // releases the native side itself.
        creating.complete();
        return;
      }
      ...
      creating.complete();
      throw CameraException(e.code, e.message ?? e.code);
    }
    _eventSubscription =
  ...
  /// 1 s / [shutterSpeed] as a frame duration in nanoseconds; 0 stays auto.
  static int _frameDurationNs(int shutterSpeed) =>
      shutterSpeed == 0 ? 0 : 1000000000 ~/ shutterSpeed;

  /// Metres to dioptres. PixelPaper patch: 0.1.0 sent `1 / 0 = ∞` for the
  /// documented "0 = auto", and the plugin then switched autofocus *off* and
  /// parked the lens at an impossible distance.
  static double _diopters(double metres) =>
      metres <= 0 || metres.isInfinite ? 0 : 1 / metres;

  /// PixelPaper patch: changes ISO, shutter speed, white balance and focus on
  /// the running session, with the same units and "0 = auto" convention as
  /// the constructor. The preview and every later [takePicture] follow.
  Future<void> setManualSettings({
    required int iso,
    required int shutterSpeed,
    required WhiteBalancePreset whiteBalance,
    ...
    if (!value.isInitialized || _isDisposed) {
      throw CameraException(
        ...
        'setManualSettings was called on uninitialized CameraController.',
      );
    }
    try {
  ...
  /// PixelPaper patch: digital zoom, from 1 to [CameraDescription.maxZoom].
  /// The preview and every later [takePicture] follow.
  Future<void> setZoom(double ratio) async {
    if (!value.isInitialized || _isDisposed) return;
    try {
      await _channel.invokeMethod<void>('setZoom', <String, dynamic>{
        'ratio': ratio,
      });
    } on PlatformException catch (e) {
      throw CameraException(e.code, e.message ?? e.code);
  ...
  Future<void> dispose() async {
    ...
    _isDisposed = true;
    super.dispose();
    final creating = _creatingCompleter;
    if (creating == null) {
      // Never initialised: there is nothing native to release.
      return;
    }
    await creating.future;
    // Always asked, even after a failed initialize(): the native side may
    // have opened the texture before the failure.
    await _channel.invokeMethod<void>(
      'dispose',
      <String, dynamic>{'textureId': value.isInitialized ? _textureId : null},
    );
